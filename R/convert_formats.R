...
popim_pop_3d <- function(region = character(),
                       year_min = integer(), year_max = integer(),
                       age_min = 0, age_max = 100) {

    popim:::assert_character(region)
    
    popim:::assert_scalar_wholenumber(year_min)
    popim:::assert_scalar_wholenumber(year_max)
    popim:::assert_non_negative(year_max - year_min)

    popim:::assert_scalar_wholenumber(age_min)
    popim:::assert_scalar_wholenumber(age_max)
    popim:::assert_non_negative(age_min)
    popim:::assert_non_negative(age_max - age_min)
    ...
    class(pop_wide) <- c("popim_population_3d", "list")

    validate_popim_pop_3d(pop_wide)
}

##' Converts from the standard (long) `popim_population` format to the 3d format
##'
##' Takes a popim_population object and converts it to the 3d format
##' that is based on 3d arrays for region / year / age.
##'
##' The S3 class `popim_pop_3d` is a list of 5 elements, the first 3
##' label the dimesions of the data, the last 3 are the 3d arrays
##' holding the data that is stored in the `immunity` and `pop_size`
##' columns of the `popim_population` object converted:
##'
...
##'   be non-negative.
##'
##' @param pop_long object of class "popim_population" to be converted
##'     to the 3d format
##' @return object of S3 class `popim_pop_3d`: list of * the labels
##'     for each dimension (region, year, age) * two 3d arrays
##'     vacc_data and pop_data holding the data stored in the columns
##'     `immunity` and `pop_size` in the input `popim_population` object
##' @seealso [convert_pop_to_3d()], [popim::popim_population()]
##' @author Tini Garske
##' @importFrom rlang .data
convert_pop_to_3d <- function(pop_long) {

    popim:::assert_population(pop_long)

    ## setting up the structure:
    pop_wide <- popim_pop_3d(region = attributes(pop_long)$region,
...
##' Convert from the `popim_population_3d` class `popim_population` class
##' (long format)
##' 
##' Takes and object of class `popim_population_3d` and converts it to
##' the standard (long format) of S3 class `popim_population` (as
##' generated by [popim::popim_population()].
##' 
##' @param pop_3d An object of class `popim_population_3d`
##' @return An object of class `popim_population`
##' @seealso [popim::popim_population()], [convert_pop_to_3d()]
##' @author Tini Garske
##' @export
convert_pop_to_long <- function(pop_3d) {

    assert_popim_pop_3d(pop_3d) ## just checking the class is correct.

    ## setting up the structure:
    pop_long <- popim::popim_population(region = pop_3d$region_labels,
